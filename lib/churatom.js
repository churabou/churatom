...
function File(uri) {
  this.name = uri.split('/')[uri.split('/').length-1]; // uri.split('/').last
  this.path = uri;
}

atom.workspace.onDidOpen((event) => {
  // https://atom.io/docs/api/v1.0.7/Workspace#instance-onDidOpen
  const indexOf = atom.history.files.map(file => file.path).indexOf(event.uri);
  if (indexOf > -1) {
      atom.history.files.splice(indexOf, 1);
  }
  atom.history.files.push(new File(event.uri));
});
